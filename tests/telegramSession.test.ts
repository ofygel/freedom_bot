...
test('resetTelegramSession performs webhook, close, and logout steps', async () => {
  const calls: string[] = [];
  ...
  assert.deepEqual(calls, ['deleteWebhook', 'close', 'logOut']);
  ...
  assert(warnMessages.includes('Logged out previous Telegram session via Telegram API.'));
  assert.equal(logs.filter((log) => log.level === 'error').length, 0);
});

test('resetTelegramSession resolves when logout succeeds after failures', async () => {
  const calls: string[] = [];
  const telegram: TelegramSessionApi = {
    deleteWebhook: async () => {
      calls.push('deleteWebhook');
...
test('resetTelegramSession reports failure when no step succeeds', async () => {
  ...
  assert(warnMessages.includes('Failed to delete previous Telegram webhook via Telegram API.'));
  assert(warnMessages.includes('Failed to close previous Telegram session with close()'));
  assert(warnMessages.includes('Failed to log out previous Telegram session with logOut()'));
