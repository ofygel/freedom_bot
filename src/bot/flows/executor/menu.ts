  ...
  const copy = getExecutorRoleCopy(state.role);
    ...
    if (access.isVerified) {
      return [
        ...
        `Документы ${copy.genitive} подтверждены. Оформите подписку, чтобы получить ссылку на канал.`,
        'Используйте кнопку «📨 Получить ссылку на канал», чтобы выбрать период и отправить чек.',
      ];
    }
